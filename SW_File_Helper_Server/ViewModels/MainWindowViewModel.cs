...
namespace SW_File_Helper_Server.ViewModels
{
    internal class MainWindowViewModel : ValidatableViewModel
    {
        ...
        public void OnMessageRecieved(string message, string clientIp)
        {
            ...
            var clientIpStyle = m_CommonResources["ClientIPStyle"] as Style;
        ...
        private void OnCommandRecieved(string value, string clientIp)
        {
            JObject obj = JObject.Parse(value);
            string commandText = obj["Text"].ToString();

            JsonSerializerSettings settings = new JsonSerializerSettings();
            settings.Formatting = Formatting.None;

            if (commandText.Equals(SW_File_Helper.Common.Constants.PROCESS_FILES_COMMAND))
            {
                //Process Files Command Recieved
                ProcessFilesCommand command = JsonHelper.DeSerialize(value, new ProcessFilesCommand(), settings);

                var file = m_processFilesCommandToFileModelConverter.Convert(command);

                m_fileProcessor.Process(new List<FileModel>() { file }, command.NewFileExtension);
            }
        }
        #endregion
    }
